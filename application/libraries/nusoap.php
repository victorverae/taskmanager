...
class nusoap_client extends nusoap_base  {
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		//echo " Estoy en  call Function de nusoap!!";
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			if (is_string($params)) {
				...
				$payload = $params;
			} elseif (is_array($params)) { //echo " HE RECIBIDO UN ARREGLO DE PARAM!!";
		...
		if ($style == 'rpc') {
			if ($use == 'literal') {
				...
				if ($namespace) { //echo ' VALOR DE PARAMETROS: '.$payload;
					// http://www.ws-i.org/Profiles/BasicProfile-1.1-2004-08-24.html R2735 says rpc/literal accessor elements should not be in a namespace
					$payload = "<$nsPrefix:$operation xmlns:$nsPrefix=\"$namespace\">" .
								$payload .
								"</$nsPrefix:$operation>";
				} else { //echo ' SIN NAMESPACE VALOR DE PARAMETROS: '.$payload;
					$payload = "<$operation>" . $payload . "</$operation>";
				}
			} else {
				$this->debug("wrapping RPC request with encoded method element");
				if ($namespace) { //echo ' LITERAL VALOR DE PARAMETROS: '.$payload;
					$payload = "<$nsPrefix:$operation xmlns:$nsPrefix=\"$namespace\">" .
								$payload .
								"</$nsPrefix:$operation>";
				} else {	//echo ' LITERAL VALOR DE PARAMETROS: '.$payload;
					$payload = "<$operation>" .
								$payload .
								"</$operation>";
				}
